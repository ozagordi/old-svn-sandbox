...
max_proc        = None  # number of processors to use in parallel (None => all available)
...
def main(fas_reads, step, win_shifts, keep_all_files, iters, alpha):
    ...
    for k, v in aligned_reads.items():
    ...
    runlist = [] # prepare all windows
    for sh in range(win_shifts):
        shift = sh * single_sh
        win_list = [ [s+shift, s+shift+step-1, aligned_reads] for s in range(1, gen_length+1, step) ]
        # print reads for this shift on all processors
        pool = Pool()
        rn_list = pool.map(print_window, win_list)
        
        for rpair in zip(win_list, rn_list):
            wstart = rpair[0][0]
            wstop = rpair[0][1]
            rn = rpair[1]
            declog.info('%d reads in %d' % (rn, wstart))
            fst = 'w%d-%d.reads.fas' % (wstart, wstop)
            # if no reads in the window, go on
            if rn <= 1:
                declog.info('No reads in window %d-%d' % (wstart, wstop))
                os.remove(fst)
                continue
            if not os.path.exists('corrected/w%d-%d.reads-cor.fas.gz' % (wstart, wstop)):
                runlist.append([fst, iters, alpha])
            else:
                declog.info('no need to run again on w%d-%d' % (wstart, wstop))
            declog.info('window %d to %d\t%d reads here' % (wstart, wstop, rn))
    ...
    # parse corrected reads and (re)move intermediate files
    proposed= {}
    for sh in range(win_shifts):
        shift = sh * single_sh
...
#            if os.path.exists(dbg_file):
            ...
            if not keep_all_files:
                ...
                if proposed[s+shift] != 'not found':
                    ...
                    os.remove(stem + '.reads-freq.csv')
                    os.remove(stem + '.reads-support.fas')
            else:
                if proposed[s+shift] != 'not found':
                    # zip and move to directories
                    try: #trying to zip the .dbg file
                    ...
                    try: #trying to zip the .smp file
                        gzf = gzip_file(stem + '.smp')
                        try:
                    ...
                    try: #trying to zip the .reads-cor file
                        gzf = gzip_file(stem + '.reads-cor.fas')
                        try:
                    ...
                    try: #trying to zip the .reads-support file
                        gzf = gzip_file(stem + '.reads-support.fas')
                        try:
                    ...
                    try: #trying to zip the .freq file
                        gzf = gzip_file(stem + '.reads-freq.csv')
                        try:
                ...
                else: # if the window has no reads remove empty dbg and smp
                    pass
#                    os.remove(stem + '.smp')
#                    os.remove(stem + '.dbg')
    ...
    in_stem = os.path.split(fas_reads)[1].split('.')[0]
    fch = open('%s.cor.fas' % in_stem, 'w')
    for r in to_correct:
        cx = to_correct[r][4].count('X')
