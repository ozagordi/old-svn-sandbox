...
int main(int argc, char** argv){
  ...
  outstr.resize(insize-1, 'm');
  outstr.resize(insize, 'p');
  */
  ...
  //   cout<<n<<endl<<J<<endl<<J/10 + 1<<endl<<endl;
   ...
   for(i=0; i<n; i++){
     readtable[i] = (crnode*)malloc(sizeof(crnode));
   ...
   */
   ...
   /*
   for(i=0; i<n; i++){
     if(readtable[i]->mindex == index){
       readtable2[index]->creads = readtable[i]->creads;
       readtable2[index]->missing = readtable[i]->missing;
       readtable2[index]->weight = it->second;
       readtable2[index]->mindex = i;
       index++;
       it++;
     }
   }
   */
   ...
   //for(i=0; i<n; i++) 
   // creads[i]  = new int[J/10 + 1];

   //for(i=0; i<n; i++)
   //creads[i] = conversion(r[i], J);
      
   //   for(i=0; i<q; i++){
     //for(j=0; j<J/10 + 1; j++)
       //cout<<readtable2[i]->creads[j]<<"  ";
     // cout<<" New Read "<<i<<" which is equivalent to Read "<<readtable2[i]->mindex<<" has  ";
     //     for(wcount=0; wcount<readtable2[i]->weight; wcount++)
     // cout<<readtable2[i]->mindices[wcount]<<"  ";
     // cout<<"weight = "<<readtable2[i]->weight<<endl;
   //   }
   ...
   for(i=0; i<q; i++){
     //for(j=0; j<J/10 + 1; j++)
     // cout<<readtable2[i]->creads[j]<<" ";
     //cout<<readtable2[i]->weight<<endl;
     //for(j=0; j<J; j++)
     //cout<<r[readtable2[i]->mindex][j];
     //cout<<endl;
     epiphany = (crnode*)malloc(sizeof(crnode));
     epiphany->creads = new int[J/10 + 1];
     epiphany = read_conversion(r[readtable2[i]->mindex], J);
     for(j=0; j<J/10 + 1; j++)
       cout<<(readtable2[i]->creads[j] != epiphany->creads[j]);
     cout<<endl;
   }
   */  
  build_assignment(stat_file);

  cout<<"Assignment built"<<endl;
  ...
  *********************/
  ...
  cout<<k<<endl<<HISTORY<<endl<<"Survived sampling"<<endl;
...
void build_assignment(std::ofstream& out_file){
  ...
  */
  ...
  //g = gsl_ran_discrete_preproc(q, p_q);
...
ssret* sample_class(unsigned int i, unsigned int step){
  ...
  ***********************************************************/
  ...
  while (cn != NULL){
	...
	nodist = cn->rd1[i];
	//dist = cn->rd0[i] * readtable2[i]->weight;
	//nodist = cn->rd1[i] * readtable2[i]->weight;
	//nodist = readtable2[i]->weight * J - dist;
	
	//log_P[st] = readtable2[i]->weight * gsl_sf_log((double)tw) + nodist * gsl_sf_log(b1) +  dist * gsl_sf_log(b2);
	log_P[st] = gsl_sf_log((double)tw) + nodist * gsl_sf_log(b1) +  dist * gsl_sf_log(b2);
	//log_P[st] = (gsl_sf_log((double)sz) + nodist * gsl_sf_log(b1) +  dist * gsl_sf_log(b2)) * readtable2[i]->weight;
  ...
  dist = p[0];
  nodist = p[1];
  //dist = p[0] * readtable2[i]->weight;
  //nodist = p[1] * readtable2[i]->weight;
  //nodist = readtable2[i]->weight * J - dist;
