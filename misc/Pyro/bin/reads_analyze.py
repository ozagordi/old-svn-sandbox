...
#            if float(r.description.split('|')[1]) > thresh and '-' not in seq_str and seq_str.count('N') < 5:
            if float(r.description.split('|')[1]) > thresh and seq_str.count('N') < 5:
                count += 1
        ...
        log_inst.info('different haplotype=%d, considered reads=%d, discarded=%d'
                      % (len(hap_tmp), sum1, len(discarded)))
...
def main():
    ...
    try:
        ...
        sys.exit('\nusage: reads_analyze.py corrected_reads original_reads\n')
