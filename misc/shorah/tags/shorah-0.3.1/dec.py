...
def main(fas_reads, step, win_shifts, keep_all_files, iters, alpha):
    ...
    if keep_all_files:
        for sd_name in ['debug', 'sampling', 'freq', 'support', 'corrected', 'raw_reads']:
            if not os.path.exists(sd_name):
                os.mkdir(sd_name)
            else:
                tr = glob.glob(sd_name+'/*')
                for trf in tr:
                    os.remove(trf)
